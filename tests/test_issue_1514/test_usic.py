...
def setUp():
    ...
    remote_dir.make_dir(unit['unit_sandbox'], flags=rs.filesystem.CREATE_PARENTS)
...
def test_transfer_single_folder_to_unit(
    ...
    actionables.append({
        ...
        'flags':    [rp.CREATE_PARENTS, rp.SKIP_FAILED, constants.RECURSIVE],
        'priority': 0
    })
...
@mock.patch.object(ru.Profiler, 'prof')
@mock.patch('radical.utils.raise_on')
def test_transfer_single_file_to_staging_area(
                                              mocked_init,
                                              mocked_method,
                                              mocked_profiler,
                                              mocked_raise_on):
    ...
    # Peek inside the remote directory to verify
    remote_dir = rs.filesystem.Directory(staging_area,
                                           session=session)
...
@mock.patch.object(ru.Profiler, 'prof')
@mock.patch('radical.utils.raise_on')
def test_transfer_single_folder_to_staging_area(
                                                mocked_init,
                                                mocked_method,
                                                mocked_profiler,
                                                mocked_raise_on):
    ...
    for x in remote_dir.list():
        if remote_dir.is_dir(x):
            child_x_dir = rs.filesystem.Directory(os.path.join(staging_area,x.path) ,
                                                    session=session)
...
@mock.patch.object(ru.Profiler, 'prof')
@mock.patch('radical.utils.raise_on')
def test_transfer_multiple_folders_to_staging_area(
                                                   mocked_init,
                                                   mocked_method,
                                                   mocked_profiler,
                                                   mocked_raise_on):
    ...
    for x in remote_dir.list():
       if remote_dir.is_dir(x):
            ...
            for y in child_x_dir.list():
                if child_x_dir.is_dir(y):
                    gchild_x_dir= rs.filesystem.Directory(os.path.join(staging_area,x.path + '/' + y.path) ,
                                                session=session)
