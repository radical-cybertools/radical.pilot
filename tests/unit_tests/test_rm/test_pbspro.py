...
def test_configure(mocked_init, mocked_raise_on, mocked_parse_pbspro_vnodes):
    ...
    os.environ['PBS_NODEFILE'] = 'tests/unit_tests/test_cases/rm/nodelist.pbs'
...
def test_parse_pbspro_vnodes(mocked_init, mocked_raise_on, mocked_configure,
                             mocked_subproc):
...
@mock.patch('subprocess.check_output',
            return_value='exec_vnode = (vnode1:cpu=3)+(vnode2:cpu=2)')
def test_parse_pbspro_vnodes_error(mocked_init, mocked_raise_on,
