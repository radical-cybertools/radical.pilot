import asyncio
import random
from ddmd_base_asyncflow import DDMD_Base

# --------------------------------------------------------------------------
#
class DDMD_manager(DDMD_Base):
    def __init__(self, execution_backend, **kwargs):
        ...
        self.workflow_loads = {
                'task_train_model': {'task_num': self._model_train_cores,   'cores_per_task': 1},
                'task_train_ff':    {'task_num': self._ff_train_cores,      'cores_per_task': 1},
                'task_md_sim':      {'task_num': self._md_sim_cores,        'cores_per_task': 1}}

        self.total_tasks = 0
        for _, load in self.workflow_loads.items():
            self.total_tasks += load['task_num']
