...
Part 2: Local Pilot Usage
*************************

One of the most important feature of Sinon is ...
...
Hands-On: Local Pilot Submission
================================

Before we discuss the individual API calls in more detail, let's get down and
dirty and run our first example: creating and running a Sinon pilot on your
local machine.

Create a new file ``sinon_example_local.py`` and paste the following code:

.. literalinclude:: ../../../examples/tutorial/sinon_example_local.py
...
.. code-block:: none
    ...
    Job State : sinon.job.Job.New

    ...starting job...

    Job ID    : [fork://localhost]-[644240]
    Job State : sinon.job.Job.Pending

    ...waiting for job...

    Job State : sinon.job.Job.Done
    Exitcode  : None
...
Once the job has completed, you will find a file mysinonjob.stdout in your current working directory. It should contain the line:
...
complexity is hidden within Sinon, it is necessary to do a lot of internal
...
environment variable ``SINON_VERBOSE`` to a value between 1 (print only critical
messages) and 5 (print all messages). Give it a try with the above example:

.. code-block:: bash

  SINON_VERBOSE=5 python sinon_example_local.py


Discussion
==========

Now that we have successfully run our first job with sinon-python, we will
discuss some of the the building blocks and details of the code.

The job submission and management capabilities of sinon-python are packaged in
the `sinon.job module (API Doc). Three classes are defined in this module:
...
In order to use the Sinon Job API, we first need to import the sinon-python
module:
...
The URL is a way to tell sinon-python what type of resource or middleware you
want to use and where it is. The URL parameter is passed to sinon-python's plug-
...
.. code-block:: python
    ...
    js = sinon.job.Service("fork://localhost")
...
.. code-block:: python
    ...
    jd = sinon.job.Description()
    
    # environment, executable & arguments
    jd.environment = {'MYOUTPUT':'"Hello from Sinon"'}       
    jd.executable  = '/bin/echo'
    jd.arguments   = ['$MYOUTPUT']

    # output options
    jd.output = "mysinonjob.stdout"
    jd.error  = "mysinonjob.stderr"
