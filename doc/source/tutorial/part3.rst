...
most important capabilities of SINON: abstracting system heterogeneity. We can
...
Copy the code from the previous example to a new file sinon_example_remote.py.
Add a ``sinon.Context`` and ``sinon.Session`` right before the ``job.Service`` 
...
.. code-block:: python
    ...
    js = sinon.job.Service("ssh://remote.host.net", session=session)

Alternatively, if you have access to a PBS cluster, use a ``pbs+ssh://...`` URL:

.. code-block:: python
  
    js = sinon.job.Service("pbs+ssh://remote.hpchost.net", session=ses)
...
As opposed to the previous "local" example, you won't find a ``mysinonjob.stdout``
...
Contexts. Contexts are used to define security / log-in contexts for SINON
...
commonly used contexts in SINON are ``ssh`` and ``gsissh``:
...
``sinon.Session`` object. A session can have one or more Contexts. At runtime,
SINON Python will iterate over all Contexts of a Session to see if any of them
...
.. code-block:: python
    ...
    js = sinon.job.Service("ssh://remote.host.net", session=ses)
