...
class DataTask(Task):
    """ 
    A DataTask represents a self-contained, related set of data.
    A DataTask is defined as an immutable container for a logical group of
    "affine" data files, e. g. data that is often accessed together
    e.g. by multiple Tasks.
    ...
    def import_data(self, src):
        """
        For a data task which does not point to PFNs yet, create a first PFN as
    ...
    def export_data(self, tgt):
        """
        Copy any of the data_task's PFNs to the tgt URL.
        """
