...
############# These are all supported options for sinon.logging #################
##
_all_logging_options = [
    { 
    'category'      : 'sinon.logging',
    'name'          : 'level', 
    'type'          : str, 
    'default'       : 'CRITICAL', 
    'valid_options' : ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],
    'documentation' : 'The log level',
    'env_variable'  : 'SINON_VERBOSE'
    },
    { 
    'category'      : 'sinon.logging',
    'name'          : 'filters', 
    ...
    'env_variable'  : 'SINON_LOG_TARGETS' 
    },
    { 
    'category'      : 'sinon.logging',
    'name'          : 'ttycolor', 
...
class _Logger (Confugurable)
    ...
    def __init__(self):
        ...
        Configurable.__init__(self, 'sinon.logging', _all_logging_options)    
        cfg = self.get_config()
...
#
def getLogger (name='sinon'):
    ''' Get a Sinon logger.  For any new name, a new logger instance will be
    created; subsequent calls to this method with the same name argument will
    return the same instance.'''

    # make sure the sinon logging configuration is evaluated at least once
    _Logger ()
