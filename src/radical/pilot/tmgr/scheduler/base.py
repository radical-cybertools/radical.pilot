...
class TMGRSchedulingComponent(rpu.Component):
    ...
    def initialize(self):
        ...
        self._tasks_lock   = ru.RLock()  # lock on the above dicts (both!)
            ...
            with self._pilots_lock:
                ...
                for pilot in pilots:
                    ...
                    if early_tasks:
                        ...
                        self._assign_pilot(early_tasks, pilot)
    ...
    # --------------------------------------------------------------------------
    #
    def _assign_pilot(self, tasks, pilot):
        '''
        ...
        for task in tasks:
            ...
            task['pilot'           ] = pid
            task['client_sandbox'  ] = sboxes['client']
            task['pilot_sandbox'   ] = sboxes['pilot']
            task['resource_sandbox'] = sboxes['resource']
            task['task_sandbox'    ] = task['description']['sandbox']

            if not task['task_sandbox']:
                task['task_sandbox'] = task['pilot_sandbox'] + '/' + task['uid']

            task['task_sandbox_path'] = ru.Url(task['task_sandbox']).path

        with self._tasks_lock:
            for task in tasks:
                self._sboxes[task['uid']] = task['task_sandbox']

        for task in tasks:

            for sd in (ru.as_list(task['description']['input_staging' ]) +
                       ru.as_list(task['description']['output_staging'])):

                # FIXME: client sandbox needs full URL (not only path)
                src_ctx = {'pwd'      : os.getcwd(),                # !!!
                           'task'     : task['task_sandbox']}
                tgt_ctx = {'pwd'      : task['task_sandbox'],       # !!!
                           'task'     : task['task_sandbox']}
        ...
        with self._tasks_lock:
            ...
            self._tasks[pid] += [task['uid'] for task in tasks]
    ...
    def work(self, tasks):
        ...
        #     when task is in self._tasks - or wait for task to appear there
        tasks = ru.as_list(tasks)
        ...
        with self._pilots_lock:
            ...
            for task in tasks:
                ...
                if pid:
                    ...
                    if pilot:
                        if pid not in to_advance:
                            to_advance[pid] = [pilot, list()]
                        to_advance[pid][1].append(task)
        ...
        if to_advance:
            all_tasks = list()
            for pilot, _tasks in to_advance.values():
                self._assign_pilot(_tasks, pilot)
                all_tasks += _tasks
            self.advance(all_tasks, rps.UMGR_STAGING_INPUT_PENDING,
                         publish=True, push=True)
