...
class ComputeUnitDescription(dict):
    ...
    **Example**::
    ...
    def verify(self):
        ...
        self.set_default(KERNEL          , '')
        self.set_default(NAME            , '')
        self.set_default(EXECUTABLE      , '')
        self.set_default(ARGUMENTS       , '')
        self.set_default(ENVIRONMENT     , '')
        self.set_default(PRE_EXEC        , '')
        self.set_default(POST_EXEC       , '')
        self.set_default(PILOT           , '')
        self.set_default(STDOUT          , '')
        self.set_default(STDERR          , '')
        self.set_default(CPU_PROCESS_TYPE, '')
        self.set_default(CPU_THREAD_TYPE , '')
        self.set_default(GPU_PROCESS_TYPE, '')
        self.set_default(GPU_THREAD_TYPE , '')

        self.set_default(CPU_PROCESSES   , 0)
        self.set_default(CPU_THREADS     , 0)
        self.set_default(GPU_PROCESSES   , 0)
        self.set_default(GPU_THREADS     , 0)
        self.set_default(MEM_PER_PROCESS , 0)
