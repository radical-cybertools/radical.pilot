...
class EVComponent(Object):
    ...
    #
    @staticmethod
    def fork(parent, name, cfg):
        '''
        perform fork/exec to instantiate a new EVComponent in a separate child
        process.  This method will return a process ID to manage the component's
        lifetime.  This method will be called by the static `start_components()`
        method.

        The method will let any exceptions occuring during fork/exec fall
        through to the caller.
        '''

        # NOTE: we need to sleep a little after the fork, to avoind this race:
        #       https://bugs.python.org/issue25666

        # NOTE: we can only fork/exec in the main thread, because of
        ...
        pid = os.fork()
        if pid:
            # parent - sleep a little
            time.sleep(1)
            return pid
        else:
            # child
            os.execlp('radical-pilot-component', parent, name, cfg)
    ...
    def create(argv):
        '''
        This method is called by `radical-pilot-component`, thus right after the
        `execlp` invocation above.  We interpret the arguments to load,
        instantiate and return the respective component class.

        The exepcted arguments are:
          - the name of the script
          - the parent name (for logging)
          - the child name
          - the name of the component config (json)

        We expect 'module' and 'class' to be specified in the config.
        '''

        if len(argv) != 4:
            assert(len(argv) == 4), 'expected 4 arguments, not %d' % len(argv)
        ...
        logger = ru.get_logger(child)
        logger.debug('%s runs %s for %s', parent, script, child)
        logger.info('loading component %s [%s]', child, cfg)

        cfg_dict = ru.read_json(cfg)

        assert('module' in cfg_dict), 'no "module" specified in %s' % cfg
        assert('class'  in cfg_dict), 'no "class"  specified in %s' % cfg

        mod = ru.load_module(cfg_dict['module'])
        cls = ru.load_class(mod, cfg_dict['class'])
        ret = cls(logger=logger, cfg=cfg_dict)
